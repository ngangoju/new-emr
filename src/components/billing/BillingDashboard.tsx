  ...
  return (
    <>
      ...
      <Dialog open={!!detailsInvoice} onOpenChange={(open) => !open && closeDetailsDialog()}>
        <DialogContent className="w-[95vw] sm:max-w-[900px] lg:max-w-[1100px] p-0 flex flex-col overflow-hidden bg-slate-50/95 backdrop-blur-3xl border-slate-200/60 shadow-2xl rounded-3xl !max-h-[95vh]">
          ...
          {detailsInvoice ? (
            <div className="flex-1 overflow-y-auto flex flex-col w-full h-full custom-scrollbar">
              {/* Premium Header */}
              <div className="relative bg-white px-8 md:px-12 py-10 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border-b border-slate-100/80 z-20 shrink-0">
                <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-indigo-50/40 via-white to-white pointer-events-none" />
                <div className="relative flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
                  <div className="space-y-1">
                    <div className="flex items-center gap-3">
                      <div className="h-2 w-2 rounded-full bg-indigo-500 animate-pulse" />
                      <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-indigo-500/80">Invoice Format</p>
                    </div>
                    <div className="mt-4 flex flex-wrap items-center gap-4">
                      <h3 className="font-mono text-4xl font-semibold tracking-tight text-slate-900">
                        INV-{detailsInvoice.invoiceNumber || detailsInvoice.id.slice(0, 8)}
                      </h3>
                      <div className="flex gap-2">
                        <Badge variant="outline" className="border-indigo-100 bg-indigo-50/50 text-indigo-700 uppercase tracking-widest text-[10px] shadow-sm px-3 py-1 rounded-full">
                  ...
                  <div className="text-left md:text-right">
                     <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-1">Total Summary Amount</p>
                     <p className="text-4xl md:text-5xl font-bold tracking-tighter text-slate-900">
              ...
              {/* Body Content */}
              <div className="flex-1 p-8 md:px-12 md:py-10 bg-slate-50/30">
                {/* Context Cards */}
                <div className="grid gap-6 md:grid-cols-2 mb-8">
                  {/* Patient Info Card */}
                  <div className="relative overflow-hidden rounded-2xl border border-slate-200/50 bg-white p-6 md:p-7 shadow-sm transition-all duration-300 hover:shadow-md hover:border-slate-300/60 ring-1 ring-slate-900/5">
                    ...
                    <div className="relative">
                      ...
                      <p className="text-2xl font-semibold text-slate-900 tracking-tight">{detailsInvoice.patient?.fullName || 'Unknown Patient'}</p>
                      
                      <div className="mt-5 grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">National ID</span> 
                          <p className="text-sm font-medium text-slate-700">{detailsInvoice.patient?.nationalId || 'Not recorded'}</p>
                        </div>
                        <div className="space-y-1">
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Phone</span> 
                          <p className="text-sm font-medium text-slate-700">{detailsInvoice.patient?.phone || 'Not recorded'}</p>
                        </div>
                        <div className="space-y-1 col-span-2 pt-2 border-t border-slate-50">
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Payment Coverage</span> 
                          <div><span className="inline-flex items-center mt-1 rounded-md bg-slate-100/80 px-2.5 py-1 text-xs font-semibold text-slate-700 border border-slate-200/50">{detailsInvoice.patient?.insurance?.provider || 'Self pay standard'}</span></div>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Encounter Info Card */}
                  <div className="relative overflow-hidden rounded-2xl border border-slate-200/50 bg-white p-6 md:p-7 shadow-sm transition-all duration-300 hover:shadow-md hover:border-slate-300/60 ring-1 ring-slate-900/5">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-purple-50/50 rounded-full blur-3xl -mr-10 -mt-10 pointer-events-none" />
                    <div className="relative">
                      <div className="flex items-center justify-between mb-5">
                        <div className="flex items-center gap-3">
                          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-purple-50 text-purple-600 border border-purple-100/50 shadow-sm">
                          ...
                          <p className="text-[11px] font-bold uppercase tracking-widest text-slate-400">Encounter Info</p>
                        </div>
                      </div>
                      
                      <div className="space-y-4">
                        <div className="flex flex-col gap-1 pb-4 border-b border-slate-50">
                           <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Attending Practitioner</span>
                           <p className="text-lg font-semibold text-slate-900">{detailsInvoice.doctorName || (detailsInvoice.doctorId ? `Doc. ${detailsInvoice.doctorId.slice(0, 8)}` : 'N/A')}</p>
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Consultation</span>
                            <p className="text-xs font-mono font-medium text-slate-600 truncate">{detailsInvoice.consultationId?.slice(0, 18) || '—'}</p>
                          </div>
                          <div className="space-y-1">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Lab Order</span>
                            <p className="text-xs font-mono font-medium text-slate-600 truncate">{detailsInvoice.labOrderId?.slice(0, 18) || '—'}</p>
                          </div>
                        </div>
                        <div className="flex items-center justify-between pt-2">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Last Revised</span>
                            <span className="text-[11px] font-medium text-slate-500 bg-slate-50 px-2 py-1 rounded-md">{format(new Date(detailsInvoice.updatedAt), 'MMM d, yy h:mm a')}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {/* Line Items Table */}
                <div className="mb-8 overflow-hidden rounded-2xl border border-slate-200/60 bg-white shadow-sm ring-1 ring-slate-900/5">
                  <div className="border-b border-slate-100 bg-slate-50/50 px-6 md:px-8 py-5 flex items-center justify-between">
                    <p className="text-sm font-semibold text-slate-900">Itemized Services</p>
                    <Badge variant="secondary" className="bg-white text-slate-600 text-[10px] uppercase font-bold tracking-widest border border-slate-200 shadow-sm px-2.5 py-0.5 rounded-full">
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full text-sm w-full">
                      <thead className="bg-white">
                        <tr>
                          <th className="px-6 md:px-8 py-4 text-left text-[10px] font-bold uppercase tracking-widest text-slate-400 border-b border-slate-100">Service Description</th>
                          <th className="px-4 py-4 text-center text-[10px] font-bold uppercase tracking-widest text-slate-400 border-b border-slate-100 w-24">Qty</th>
                          <th className="px-4 py-4 text-right text-[10px] font-bold uppercase tracking-widest text-slate-400 border-b border-slate-100 w-32">Unit Price</th>
                          <th className="px-6 md:px-8 py-4 text-right text-[10px] font-bold uppercase tracking-widest text-slate-400 border-b border-slate-100 w-40">Total</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-50/80 bg-white">
                         {detailsInvoice.items.length ? detailsInvoice.items.map((item, index) => (
                          <tr key={item.id || `${item.tariffId}-${index}`} className="group transition-colors hover:bg-indigo-50/30">
                            <td className="px-6 md:px-8 py-4.5">
                              <p className="font-semibold text-slate-900">{item.tariff?.serviceName || item.tariff?.billingCode || 'Unnamed service'}</p>
                              {item.tariff?.billingCode && <p className="text-[11px] text-slate-400 font-mono mt-1 w-fit bg-slate-50 px-1.5 rounded">{item.tariff.billingCode}</p>}
                            </td>
                            <td className="px-4 py-4.5 text-center">
                              <span className="inline-flex h-7 min-w-[2rem] items-center justify-center rounded-lg bg-slate-50 px-2 text-xs font-semibold text-slate-600 ring-1 ring-inset ring-slate-200/50">
                                {item.quantity}
                              </span>
                            </td>
                            <td className="px-4 py-4.5 text-right font-medium text-slate-500 tabular-nums">
                               {item.unitPrice.toLocaleString()}
                            </td>
                            <td className="px-6 md:px-8 py-4.5 text-right tabular-nums">
                          ...
                          <tr>
                            <td colSpan={4} className="px-6 py-16 text-center bg-slate-50/30">
                              <div className="flex flex-col items-center justify-center space-y-3">
                                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-white shadow-sm ring-1 ring-slate-200 text-slate-400">
                ...
                <div className="grid gap-6 lg:grid-cols-[1fr_400px]">
                  {/* Payments */}
                  <div className="rounded-2xl border border-slate-200/60 bg-white p-6 md:p-7 shadow-sm ring-1 ring-slate-900/5 flex flex-col">
                    <p className="text-[11px] font-bold uppercase tracking-widest text-slate-400 mb-6 flex items-center gap-2">
                    ...
                    <div className="flex-1 space-y-3">
                      {detailsPayments.length ? detailsPayments.map((payment) => (
                        <div key={payment.id} className="relative flex items-center gap-4 rounded-xl border border-slate-100 bg-emerald-50/30 p-4 transition-all hover:bg-emerald-50/50 hover:border-emerald-200/50">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="inline-flex items-center rounded-md bg-white px-2 py-1 text-[9px] font-bold uppercase tracking-widest text-slate-500 shadow-sm border border-slate-200/80">
                            ...
                            <div className="mt-1.5 flex items-center justify-between">
                                ...
                                <div className="flex gap-2">
                                  {payment.receiptNumber && (
                                    <p className="text-[10px] text-slate-400">RCPT: <span className="text-slate-600 font-medium font-mono">{payment.receiptNumber}</span></p>
                                  )}
                                </div>
                            </div>
                          </div>
                        ...
                        <div className="flex h-full min-h-[160px] flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50/50 p-6 text-center">
                          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-white shadow-sm ring-1 ring-slate-100 mb-4">
                          ...
                          <p className="text-sm font-semibold text-slate-600">Awaiting Payments</p>
                          <p className="text-[11px] text-slate-400 mt-1 max-w-[200px] leading-relaxed">Transactions directed to this invoice will be historically logged here.</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl bg-slate-900 border border-slate-800 p-6 md:p-8 shadow-xl flex flex-col justify-between relative overflow-hidden text-slate-100">
                    {/* Background accent */}
                    ...
                    <div className="relative z-10">
                        <p className="text-[11px] font-bold uppercase tracking-widest text-slate-400 mb-6">Financial Summary</p>
                        <div className="space-y-4">
                          <div className="flex items-center justify-between pb-4 border-b border-slate-700/50">
                              <span className="text-sm font-medium text-slate-300">Subtotal Amount</span>
                              <span className="text-sm font-medium text-white tabular-nums">RWF {detailsSubtotal.toLocaleString()}</span>
                          </div>
                          {detailsDiscount > 0 && (
                              <div className="flex items-center justify-between pb-4 border-b border-slate-700/50">
                                  <span className="text-sm font-medium text-slate-300">Adjustment / Discount</span>
                                  <span className="text-sm font-medium text-emerald-400 tabular-nums">- RWF {detailsDiscount.toLocaleString()}</span>
                              </div>
                          )}
                          <div className="flex items-center justify-between pb-4 border-b border-slate-700/50">
                              <span className="text-sm font-medium text-slate-300">Covered by Insurance</span>
                              <span className="text-sm font-medium text-indigo-300 tabular-nums">RWF {detailsInsuranceDue.toLocaleString()}</span>
                          </div>
                        </div>

                        {detailsInvoice.discountReason && (
                        <div className="mt-5 rounded-xl bg-slate-800/80 border border-slate-700 p-4 backdrop-blur-sm">
                            <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-2 flex items-center gap-1.5">
                    ...
                    <div className="mt-8 pt-6 border-t border-slate-700 relative z-10">
                        <div className="flex flex-col gap-3">
                            <div className="flex items-center justify-between">
                                <span className="text-[12px] font-bold uppercase tracking-widest text-slate-400">Total Patient Due</span>
                                {detailsPatientDue > 0 ? (
                            ...
                            <span className="text-4xl lg:text-5xl font-bold tracking-tighter text-white tabular-nums drop-shadow-sm text-right">
                              RWF {detailsPatientDue.toLocaleString()}
                            </span>
                        </div>
                    </div>
                  </div>
              ...
              {/* Sticky Footer */}
              <div className="sticky bottom-0 z-20 flex shrink-0 items-center justify-end gap-4 border-t border-slate-200/60 bg-white/80 px-8 py-5 backdrop-blur-xl shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.05)]">
                <Button variant="outline" className="font-semibold px-6 border-slate-200 text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-colors h-11 rounded-xl" onClick={closeDetailsDialog}>Close Preview</Button>
                {detailsPatientDue > 0 && (
                  <Button className="font-semibold px-8 h-11 bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-600/20 transition-all rounded-xl" onClick={() => {
                    closeDetailsDialog()
