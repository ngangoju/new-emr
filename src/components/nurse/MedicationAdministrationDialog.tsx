  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <WorkspaceModalShell>
        ...
        {/* ─── Scrollable Body ─── */}
        {admission && (
          <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 bg-muted/50">
                ...
                <div className="rounded-xl border bg-card p-6 shadow-sm">
                  ...
                  {medicationOptions.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-muted/50 p-10 text-center">
                      <div className="flex h-12 w-12 items-center justify-center rounded-full bg-card shadow-sm ring-1 ring-border mb-3 text-muted-foreground">
                        <Pill className="h-5 w-5" />
                      </div>
                      <p className="text-sm font-medium text-muted-foreground">No dispensed medications</p>
                      <p className="text-[11px] text-muted-foreground mt-1 max-w-[240px] leading-relaxed">Pharmacy needs to fulfill the order before you can document administration events.</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      <div className="space-y-1.5">
                        <Label htmlFor="medication-line" className="text-xs font-semibold text-foreground">Medication Line</Label>
                      ...
                      {selectedMedication && (
                        <div className="rounded-xl border border-border bg-muted/50 p-4">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <p className="font-semibold text-foreground">{selectedMedication.item.drugName}</p>
                      ...
                      <div className="grid gap-3 sm:grid-cols-3">
                        <div className="space-y-1.5">
                          <Label htmlFor="administration-status" className="text-xs font-semibold text-foreground">Outcome</Label>
                          <Select
                        ...
                        <div className="space-y-1.5">
                          <Label htmlFor="quantity" className="text-xs font-semibold text-foreground">Quantity</Label>
                          <Input
                            id="quantity"
                            type="number"
                            min={1}
                        ...
                        <div className="space-y-1.5">
                          <Label htmlFor="administered-at" className="text-xs font-semibold text-foreground">Time</Label>
                          <Input
                            id="administered-at"
                            type="datetime-local"
                            className="h-10"
                      ...
                      <div className="space-y-1.5">
                        <Label htmlFor="reason" className="text-xs font-semibold text-foreground">
                          Reason {administrationStatus !== 'administered' ? <span className="text-rose-500">(required)</span> : <span className="text-muted-foreground">(optional)</span>}
                        </Label>
                        <Textarea
                          id="reason"
                          rows={2}
                      ...
                      <div className="space-y-1.5">
                        <Label htmlFor="notes" className="text-xs font-semibold text-foreground">Nursing Notes</Label>
                        <Textarea
                          id="notes"
                          rows={2}
                          value={notes}
                      ...
                      {administrationStatus === 'administered' && (!scannedPatientBarcode || !scannedMedicationBarcode) && (
                         <div className="flex items-center justify-center gap-2 rounded-md border border-warning/40 bg-warning-muted px-4 py-2.5 text-xs font-medium text-warning-foreground">
                ...
                <div className="flex flex-col rounded-xl border bg-card shadow-sm">
                  <div className="flex items-center justify-between px-6 py-4 border-b">
                    <div className="flex items-center gap-3">
                      <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-warning-muted text-warning-foreground">
                          ...
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                <p className="font-semibold text-foreground text-sm truncate">{entry.drugName}</p>
                      ...
                      <div className="max-h-[260px] space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {administrations.map((record) => (
                          <div key={record.id} className="rounded-lg border p-3.5 hover:bg-accent/50 transition-colors">
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                <p className="font-semibold text-foreground text-sm truncate">{record.drugName}</p>
                            ...
                            {record.reason && (
                              <p className="mt-2 text-xs text-muted-foreground bg-muted rounded-lg px-3 py-2 border border-border">
                                <span className="font-medium text-muted-foreground">Reason:</span> {record.reason}
                              </p>
                            )}
                            {record.notes && (
                              <p className="mt-2 text-xs text-muted-foreground bg-info-muted/50 rounded-lg px-3 py-2 border border-info/20/50">
                                {record.notes}
                              </p>
                            )}
                          </div>
